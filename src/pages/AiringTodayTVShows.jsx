...
import { getAiringTodayTVShows } from '../services/api'
...
const AiringTodayTVShows = () => {
    ...
    useEffect(() => {
        const loadShows = async () => {
            try {
                const results = await getAiringTodayTVShows()
                setShows(results.slice(0, 30))
                setError(null)
            } catch (err) {
                console.error(err)
                setError("Failed to load airing today TV shows")
            } finally {
                setLoading(false)
            }
        }
    ...
    return (
        <div className="media-grid-page">
            <h1 className="page-title">TV Shows Airing Today</h1>
